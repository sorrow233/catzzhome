...
export async function fetchRemote(startUrl, { kind = 'html', fetcher = globalThis.fetch } = {}) {
...
function extractTags(html, tag) {
  ...
  return [...html.matchAll(expression)].map((match) => Object.fromEntries([...match[1].matchAll(/([:\w-]+)\s*=\s*(?:"([^"]*)"|'([^']*)'|([^\s"'=<>`]+))/g)].map((attribute) => [attribute[1].toLowerCase(), attribute[2] ?? attribute[3] ?? attribute[4] ?? ''])));
}
