...
export class IconResolver {
  constructor(cache, metadata) { this.cache = cache; this.metadata = metadata; }
  ...
  async resolve(bookmark, { refresh = false } = {}) {
    ...
    const result = await firstValid(candidates);
    if (result) {
      const resolved = { type: 'image', ...result, alt: bookmark.name };
      await this.cache.set(bookmark.url, resolved);
      return resolved;
    }
...
async function firstValid(candidates) {
  const unique = [...new Map(candidates.map((item) => [item.src, item])).values()].slice(0, 9);
...
function loadImage(src, timeout = 5000) {
  return new Promise((resolve) => {
    ...
    image.onload = () => finish(image.naturalWidth > 4 && image.naturalHeight > 4);
