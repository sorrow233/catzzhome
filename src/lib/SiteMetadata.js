...
export class SiteMetadata {
  ...
  async request(url) {
    ...
    for (let attempt = 0; attempt < 2; attempt += 1) {
      try {
        const response = await fetchWithTimeout(this.fetcher, `/api/site-meta?url=${encodeURIComponent(url)}`, 6500);
        ...
        const result = {
          ...
          icons: [...new Set((Array.isArray(data.icons) ? data.icons : []).map(normalizeUrl).filter(Boolean))].slice(0, 8)
        };
...
export function proxiedIconUrl(url) {
  return `/api/site-icon?url=${encodeURIComponent(url)}`;
}
...
async function fetchWithTimeout(fetcher, url, timeout) {
  const controller = new AbortController();
  const timer = window.setTimeout(() => controller.abort(), timeout);
  try {
    return await fetcher(url, { signal: controller.signal, headers: { Accept: 'application/json' } });
  } finally {
...
function writeCache(key, data) {
  try { localStorage.setItem(`${CACHE_PREFIX}${key}`, JSON.stringify({ cachedAt: Date.now(), data })); }
  catch { /* Private browsing can continue without persistent metadata cache. */ }
}
