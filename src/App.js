...
export class App {
  ...
  async mount() {
    ...
    document.body.dataset.density = this.settings.preferences.density;
    ...
    this.bookmarks = new BookmarkComponent({
      ...
      dialogElement: this.view.querySelector('[data-bookmark-dialog]'), iconResolver: new IconResolver(this.iconCache),
    ...
    this.workspace = new Workspace({ root: this.view, settings: this.settings, onChange: (partial) => this.update(partial), announce: (message) => this.announce(message) });
    this.workspace.mount();
    this.clockWeather = new ClockWeather({ root: this.view, settings: this.settings, onSettings: () => this.settingsPanel.open('general'), announce: (message) => this.announce(message) });
    this.clockWeather.start();

    this.installer = new PwaInstaller();
    this.settingsPanel = new SettingsPanel({
      root: this.view, settings: this.settings, wallpapers: HERO_CONFIG.wallpapers, wallpaperController: this.wallpaper, assetStore: this.assetStore,
      bookmarks: this.bookmarks, clockWeather: this.clockWeather, installer: this.installer, onChange: (partial) => this.update(partial),
      onRestore: (settings) => this.applySettings(settings), onDeleteLocal: () => this.deleteLocal(), onDeleteCloud: () => this.sync.deleteCloud(), announce: (message) => this.announce(message)
    });
    ...
    this.command = new CommandCenter({
      ...
      commands: {
        focus: (argument) => this.workspace.startFocus(Number(argument) || this.settings.focus.minutes),
        note: (argument) => argument ? this.workspace.addNote(argument) : this.workspace.open('notes'),
        weather: async () => { if (await this.clockWeather.enableWeather()) this.update({ weather: this.settings.weather }); },
        settings: () => this.settingsPanel.open(),
    ...
    this.onboarding = new Onboarding({ root: this.view, settings: this.settings, onComplete: (partial) => this.update(partial), enableWeather: () => this.clockWeather.enableWeather() });
  ...
  async applySettings(remote) {
    ...
    const merged = {
      ...current, ...remote,
      search: { ...current.search, ...remote.search }, weather: { ...current.weather, ...remote.weather }, focus: { ...current.focus, ...remote.focus },
    ...
    this.bookmarks.setData(this.settings); this.workspace.renderAll(); this.settingsPanel.syncControls();
  }

  async hasCustomWallpaper() {
    try { return Boolean(await this.assetStore.get('custom-wallpaper')); }
    catch { return false; }
  }
  ...
  cloudSettings() {
    ...
    settings.customWallpaper = { enabled: false, name: '' };
    return settings;
  }
